...
let reuseDiscoverLargeCell:String = "DiscoverLargeCell"

class DiscoverLargeCell: DiscoverCell {
    ...
    */
    ...
    override func setUI()
    {
        ...
        self.productTitleLabel = UILabel(frame: CGRectMake(originXY, currentView.bottom, (currentView.width - originXY * 2), heightTitle))
    ...
    override func reloadDataWithModel(model data:DiscoverModel)
    {
        if let model:DiscoverModel = data
        {
            ...
            let read:String = String(" \(model.productRead)次观看")
